...
/**
 * Easy_Shuffle_Widget_Init Class
 *
...
class Easy_Shuffle_Widget_Init
{
	 ...
	 *
	 * @see Easy_Shuffle_Widget_Init::register_widget()
	 *
	 * @access public
	 *
	 * @since 1.0.0
	...
	{
		register_widget( 'Widget_Easy_Shuffle' );
	}
	 ...
	 *
	 * @see Easy_Shuffle_Widget_Init::admin_scripts()
	 * @see Easy_Shuffle_Widget_Init::admin_styles()
	 *
	 * @access public
	 *
	 * @since 1.0.0
	 ...
	 *
	 * @see Easy_Shuffle_Widget_Init::maybe_store_css()
	 *
	 * @access public
	 *
	 * @since 1.0.0
	...
	{
		add_action( 'eshuflw_update_widget', array( $this, 'maybe_store_css' ), 0, 4 );
		add_action( 'customize_save_widget_easy-shuffle-widget', array( $this, 'maybe_store_css' ), 0, 1 );
	}
	...
	public function maybe_store_css( $widget, $instance = array(), $new_instance = array(), $old_instance = array() )
	{
		...
		if( 'customize_save_widget_easy-shuffle-widget' === $current_filter ){
		...
		if( 'eshuflw_update_widget' === $current_filter && is_customize_preview() ){
			return;
		}
		...
		if( ! empty ( $instance['css_default'] ) ) {
			Easy_Shuffle_Widget_Utils::stick_css( $widget_id );
		} else {
	 ...
	 *
	 * @see Easy_Shuffle_Widget_Init::enqueue_front_styles()
	 *
	 * @access public
	 *
	 * @since 1.0.0
	...
	public function front_styles()
	{
		...
		foreach( $widgets as $widget_id ) {
			if( is_active_widget( '', $widget_id, 'easy-shuffle-widget', true ) ) {
				$enqueue = true;
			}
		}
